...
const inputType = (a: any): string => typeof(a);
...
const ObjWithNewKey = (a: string): object => {
    return {key: a};
}
...
????????????????????????????????????????????????????????????????????????????????????????????????????????? How to set an order?????? and why there is no 
*/ 
...
// const keyWithValueOfString = (a: newObj14[], b: newObj14[], c?: string): newObj14[] => {
